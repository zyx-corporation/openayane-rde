#!/usr/bin/env bash
# Create GitHub issues from docs/64 backlog (requires: gh auth login).
# Usage:
#   ./scripts/create_post_phase6_issues.sh           # create all issues
#   ./scripts/create_post_phase6_issues.sh --dry-run # print gh commands only
set -euo pipefail

REPO="${GITHUB_REPOSITORY:-zyx-corporation/openayane-rde}"
DRY_RUN=false
if [[ "${1:-}" == "--dry-run" ]]; then
  DRY_RUN=true
fi

if ! command -v gh >/dev/null 2>&1; then
  echo "gh CLI not found. Install: https://cli.github.com/"
  exit 1
fi

run() {
  if $DRY_RUN; then
    printf '%q ' "$@"
    echo
  else
    "$@"
  fi
}

# Optional: ensure labels exist (ignore errors if no permission)
ensure_label() {
  if $DRY_RUN; then
    echo "# gh label create \"$1\" --repo \"$REPO\" 2>/dev/null || true"
    return
  fi
  gh label create "$1" --repo "$REPO" 2>/dev/null || true
}

for L in documentation testing ci schema api adapter benchmark research paper; do
  ensure_label "$L"
done

create_issue() {
  local title="$1"
  local labels="$2"
  local body="$3"
  run gh issue create --repo "$REPO" --title "$title" --label "$labels" --body "$body"
}

# --- Epic A ---
create_issue \
  "テスト監査: RDEResult と PolicyDecision の検証境界を明示化する" \
  "testing" \
  "## 概要
\`docs/63_openayane_rde_testing_policy.md\` §3 に従い、既存テストで RDE 分類と Policy 行動を混同していないか棚卸しし、必要ならアサーション分割・コメントで境界を明示する。

## 完了条件
- [ ] 主要 golden / 分類テストで「何を検証しているか」が RDE / Policy のどちらか読み取れる
- [ ] 方針に反するテストがあれば修正または \`known_limitations\` への言及
- [ ] 関連: #99

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (A1)"

create_issue \
  "Golden / 単体テストに docs/63 テスト方針への参照コメントを追加する" \
  "documentation,testing" \
  "## 概要
テストファイル先頭または該当ケース付近に、\`docs/63\`（RDE と Policy の分離、golden の意味、自己申告の位置づけ）への短い参照を追加する。冗長にならないようモジュール単位でまとめてもよい。

## 完了条件
- [ ] \`tests/golden/\` および分類・ポリシーに触れる \`tests/unit/\` に参照が一貫してある
- [ ] 関連: #99

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (A2)"

create_issue \
  "自己申告ミスマッチ系フィクスチャの棚卸しと不足ケースの Issue 化" \
  "testing" \
  "## 概要
\`docs/63\` §4 に沿い、SelfReport と StructuralDiff の不一致を表すフィクスチャの有無・カバー範囲を一覧化し、不足があれば追補 Issue を切る。

## 完了条件
- [ ] 棚卸し結果がコメントまたは短い \`docs/\` 節（任意）に残る
- [ ] ギャップがあればフォローアップ Issue へのリンク
- [ ] 関連: #99

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (A3)"

create_issue \
  "CI に openayane-rde schema validate と golden run を明示ステップとして追加する" \
  "ci" \
  "## 概要
\`.github/workflows/ci.yml\` は現状 \`pytest\` / ruff / mypy のみ。\`openayane-rde schema validate\` と \`golden run\` を CI ステップとして明示し、CLI 回帰も固定する（既に pytest 内で同等なら、その旨を CI コメントで明文化してもよい）。

## 完了条件
- [ ] CI で schema validate と golden run が実行される、または同等カバレッジがコードコメントで説明される
- [ ] 失敗時のログがローカル再現手順と一致する

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (A4)"

create_issue \
  "RelationStore トップレベル schemas/relation_store.schema.json の採用と spec 整合" \
  "schema,documentation" \
  "## 概要
\`docs/62_openayane_rde_phase6_completion_report.md\` §5 にあるとおり、RelationStore 全体のトップレベル JSON Schema が未採用。\`specs/relation_store_schema.md\` と Pydantic / 既存スキーマ群との整合を取り、\`schemas/relation_store.schema.json\` を追加または更新する。

## 完了条件
- [ ] トップレベル schema がリポジトリに存在し、単体または schema テストで参照される
- [ ] spec 文書と矛盾がない

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (A5)"

create_issue \
  "docs/60 Phase6 計画書の Issue 番号追記とメンテ方針" \
  "documentation" \
  "## 概要
\`docs/60_openayane_rde_phase6_issue_branch_plan.md\` の「次回更新」方針に従い、GitHub Issue 作成済みの項目には Issue 番号を表に反映する。Phase 6 が PR スタックだった場合の扱いも一言メモする。

## 完了条件
- [ ] 計画書内の追跡表が現状と一致
- [ ] 今後の番号追記ルールが文書上明確

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (A6)"

create_issue \
  "CONTRIBUTING（または README）に CHANGELOG / 互換ポリシー (docs/51) への導線を追加する" \
  "documentation" \
  "## 概要
コントリビュータがリリース時に \`docs/51_openayane_rde_release_compatibility_policy.md\` と CHANGELOG を更新しやすいよう、\`CONTRIBUTING.md\` を新規に置くか、\`README.md\` に同等の短い導線とチェックリストを追加する。

## 完了条件
- [ ] 新規 PR で互換・CHANGELOG の確認が手順に含まれる

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (A7)"

# --- Epic B ---
create_issue \
  "API: ローカル評価用の最小 HTTP エンドポイント実装（設定で有効化）" \
  "api" \
  "## 概要
\`src/openayane_rde/api/app.py\` のスケルトンを超え、設定（例: openayane.toml または環境変数）で有効化される **最小** の評価 API を実装する。本番認証・高保証は対象外（\`specs/known_limitations.md\` 整合）。

## 完了条件
- [ ] ローカルで文書化された手順から呼び出せる
- [ ] 既定は従来どおり安全側（無効または local-only）

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (B1)"

create_issue \
  "API: 契約テスト・エラーレスポンス・501 からの移行方針" \
  "api,testing" \
  "## 概要
評価 API（関連 Issue B1）とあわせ、ステータスコード・エラーペイロード・OpenAPI 有無を決め、契約テストで固定する。既存 501 エンドポイントの廃止／移行表を README または api モジュール doc に残す。

## 完了条件
- [ ] 契約テストが CI を通る
- [ ] 破壊的変更は \`docs/51\` に照らして記録

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (B2)"

create_issue \
  "GitHub PR アダプタ: ライブ投稿 (opt-in) の設計スパイクとリスク記録" \
  "adapter,documentation" \
  "## 概要
Phase 5 完了レポートの「GitHub live posting（明示 opt-in）」を前進させる前に、権限・レート制限・誤投稿リスク・監査ログとの関係を 1 本の設計メモ（\`docs/\`）にまとめる。実装は別 Issue でも可。

## 完了条件
- [ ] opt-in 時の脅威と非目標が明文化されている
- [ ] 実装 Issue への参照がある

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (B3)"

create_issue \
  "ベンチマーク: 多言語小規模フィクスチャ追加（外部主張なし）" \
  "benchmark,research" \
  "## 概要
\`benchmarks/\` に小さな多言語フィクスチャを追加し、\`evaluate.py\` で回せるようにする。\`benchmarks/METRICS.md\` に集合外一般化を主張しない旨を維持・追記する。

## 完了条件
- [ ] 新フィクスチャが評価ハーネスに載る
- [ ] 非主張スコープが文書で明確

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (B4)"

create_issue \
  "ベンチマーク: 敵対的/ストレス系フィクスチャの方針と初回セット" \
  "benchmark,research" \
  "## 概要
敵対的・ストレス系（長文、境界値、意図的な schema 逸脱など）の方針を \`benchmarks/README.md\` または \`specs/known_limitations.md\` と整合させ、初回の最小セットを追加する。

## 完了条件
- [ ] 方針文書とフィクスチャが対応づけされている
- [ ] CI / evaluate で再現可能

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (B5)"

create_issue \
  "論文ドラフトと Phase 6 ベースライン報告の数値・図表の同期" \
  "paper,documentation" \
  "## 概要
\`papers/openayane_rde_paper_draft.md\` の記述を \`reports/phase6_baseline_evaluation.md\` および \`reports/phase6_baseline_eval.json\` と突合し、数値・用語・図表参照を一致させる。

## 完了条件
- [ ] 論文ドラフト内の定量・ファイル参照がレポートと矛盾しない
- [ ] 主張スコープが \`known_limitations\` と矛盾しない

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (B6)"

create_issue \
  "benchmarks/README: 評価プロトコル拡張手順の明文化" \
  "benchmark,documentation" \
  "## 概要
新しいベンチマーク単位やメトリクスを追加する際の手順（ディレクトリ規約、期待 JSON、evaluate.py への登録、METRICS 更新）を \`benchmarks/README.md\` に追記する。

## 完了条件
- [ ] コントリビュータが手順だけで拡張できる

---
_Backlog:_ \`docs/64_openayane_rde_post_phase6_project_issues.md\` (B7)"

if $DRY_RUN; then
  echo "# Dry-run complete. Run without --dry-run after: gh auth login -h github.com"
else
  echo "Created issues in $REPO. Add them to your GitHub Project board."
fi
